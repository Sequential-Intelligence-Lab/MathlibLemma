...
lemma average_action_eq_projection
    {V : Type w} [AddCommMonoid V] [Module k V]
    (ρ : Representation k G V) :
    (ρ.asAlgebraHom (GroupAlgebra.average k G)).comp
      (ρ.asAlgebraHom (GroupAlgebra.average k G))
      = ρ.asAlgebraHom (GroupAlgebra.average k G) := by
  sorry
