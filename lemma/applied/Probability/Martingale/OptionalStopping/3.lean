...
theorem Submartingale.expected_stoppedValue_mono_of_ae_le
  ...
  -- From a.e. equality of stopping times, deduce a.e. equality of stopped values
  have h_stopped_ae : stoppedValue f τ =ᵐ[μ] stoppedValue f π := by
    refine h_ae.mono ?_
    intro ω hω
    -- unfold the definition of stoppedValue and use hω : τ ω = π ω
    simp [stoppedValue, hω]
  -- Integrals of a.e. equal functions are equal
  exact (integral_congr_ae h_stopped_ae)