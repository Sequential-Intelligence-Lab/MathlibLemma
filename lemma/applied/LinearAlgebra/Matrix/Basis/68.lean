...
lemma toMatrixEquiv_neg
  ...
  toMatrixEquiv (fun i ↦ - v i) =
    - toMatrixEquiv v := by
  -- Rewrite the argument as `-v`, which is definitionally `fun i ↦ - v i`.
  change toMatrixEquiv (-v) = - toMatrixEquiv v
  -- Now use linearity: `e (-x) = - e x`.
  simpa using toMatrixEquiv.map_neg v