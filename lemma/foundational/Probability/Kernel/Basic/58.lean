...
lemma Kernel.piecewise_comp_left
  ...
  -- extensionality on kernels: point `a : α` and measurable set `t : Set γ`
  ext a t ht
  -- case split on membership of `a` in `s`
  by_cases hmem : a ∈ s
  · -- when a ∈ s, both sides reduce to (η ∘ₖ κ₁) a t
    simp [Kernel.piecewise, hs, hmem, Kernel.comp, ht]
  · -- when a ∉ s, both sides reduce to (η ∘ₖ κ₂) a t
    simp [Kernel.piecewise, hs, hmem, Kernel.comp, ht]
