...
lemma Bundle.contMDiffAt_proj_coord
    ...
    (x : TotalSpace F E) :
    ContMDiffAt (IB.prod 𝓘(𝕜, F)) 𝓘(𝕜, EB) n
      (fun y ↦ (extChartAt IB x.proj (π F E y))) x := by
  -- π is `C^n` at `x`
  have hπ :
      ContMDiffAt (IB.prod 𝓘(𝕜, F)) IB n (π F E) x := by
    simpa using (Bundle.contMDiffAt_proj (IB := IB) (p := x) (n := n))
  -- the chart map `extChartAt IB x.proj` is `C^n` at `x.proj`
  have hchart :
      ContMDiffAt IB 𝓘(𝕜, EB) n (extChartAt IB x.proj) x.proj := by
    simpa using
      (contMDiffAt_extChartAt (I := IB) (x := x.proj) (n := n))
  -- composition gives the desired result
  simpa [Function.comp] using hchart.comp x hπ