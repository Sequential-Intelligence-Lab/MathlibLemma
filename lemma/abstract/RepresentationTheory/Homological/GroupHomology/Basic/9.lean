...
lemma Rep_Tor_map_monoidal_whisker
  ...
  -- `Rep.Tor` is defined as the left derived functor of `Rep.coinvariantsTensor`
  -- in the first variable, so we can unfold it and reduce to a definitional equality.
  simpa [Rep.Tor]