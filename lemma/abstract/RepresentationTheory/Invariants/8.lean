...
lemma average_smul_average (a : k) :
    a • GroupAlgebra.average k G =
      (algebraMap k (MonoidAlgebra k G) a) * GroupAlgebra.average k G := by
  -- This is just the standard formula expressing scalar multiplication via `algebraMap`.
