...
/-- The cgf uniquely determines the law of `X` if the mgf is finite on a neighborhood of `0`.
  ...
  NOTE: With the current dummy definition `MyCgf := 0`, this statement is *not* provable
  in Lean without additional axioms. We therefore leave it as `by sorry` so that the file
  still compiles, but logically this is an admitted lemma.
-/
