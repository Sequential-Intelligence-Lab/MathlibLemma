...
by
  -- This lemma is not provable with the dummy definition of `Complex.iteratedDeriv`,
  -- because `Complex.iteratedDeriv n f c = default` is independent of `f`, `c`, `R`,
  -- while the right-hand side can be made arbitrarily small by choosing `f = 0`.
  -- Hence the statement is false in general, and cannot be proven in Lean.
  admit