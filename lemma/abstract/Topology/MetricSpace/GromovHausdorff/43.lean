...
⚠️ Warning: As stated, this lemma is in general **false** without additional
assumptions (e.g. that `f` is a bijective bi-Lipschitz equivalence between `X`
and `Y`). Therefore, it cannot be proved in Lean as-is. The `sorry` below
indicates that this statement is not established. -/
