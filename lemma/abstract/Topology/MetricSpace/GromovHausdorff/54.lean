...
lemma GromovHausdorff.covering_numbers_approx
  ...
  -- This lemma is not derivable from the given assumptions (and is in fact
  -- false in general), so it cannot be proved in Lean without additional
  -- hypotheses or using placeholders. We leave it as `sorry`.
  sorry